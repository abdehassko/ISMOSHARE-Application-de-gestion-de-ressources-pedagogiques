...
if ($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    if (($image['error']==0)){
        ...
        elseif(empty($msgerror)){
            ...
            try {
                if ($role=='admin' || $role=='formateur') {
                    $insciption=$db->prepare("UPDATE UTILISATEUR set NOM_USER=?,PRENOM_USER=?,EMAIL_INSTITUTIONNEL=?,CHEMIN_PHOTO=? where ID_USER =?");
                    $r=$insciption->execute([$Name,$lastname,$Email,"/ISMOSHARE/uploaded_files/$image[name]",$id_user]);
                    if (!empty($mdp)) {
                        if(!password_verify($mdp,$M))$mdp=password_hash($mdp,PASSWORD_ARGON2I);else $mdp=$M ;
                        $nvmd=$db->prepare("UPDATE UTILISATEUR set MOT_DE_PASSE=? WHERE ID_USER = ?");
                        $nvmd->execute([$mdp,$id_user]);
                    }
                    if ($r){
                        ...
                        $p=$prog->execute([$id_user,$Filière]);
                    ...
                    if ($r){
                        ...
                        $prog=$db->prepare("INSERT INTO MEMBRE(ID_USER, ID_FILIERE) VALUES (?, ?)");
                        $p=$prog->execute([$id_user,$Filière]);
                        $update=$db->prepare("UPDATE UTILISATEUR SET ETAT_COMPTE = 'en attente' WHERE ID_USER = ?");
                        ...
                        foreach ($admins as $admin) {
                            ...
                            $notif->execute([$admin['ID_USER'], "✍🏻 $nom $prenom a demande de modifier son profile"]);
                        }

                        $msgsucces="Demande Modification envoyee vers l admin";
                        header("Location:/ISMOSHARE/pages/connexion/page-conexion.php?mod=$msgsucces");
                        exit;
                    }
                        else $msgecche= "Erreur Modification Donnes non insere";               
                }
        ...
        if(empty($msgerror)){
            ...
            try {
                if ($role=='admin' || $role=='formateur') {
                    $insciption=$db->prepare("UPDATE UTILISATEUR set NOM_USER=?,PRENOM_USER=?,EMAIL_INSTITUTIONNEL=? WHERE ID_USER = ?");
                    $r=$insciption->execute([$Name,$lastname,$Email,$id_user]);
                    if (!empty($mdp)) {
                        if(!password_verify($mdp,$M))$mdp=password_hash($mdp,PASSWORD_ARGON2I);else $mdp=$M ;
                        $nvmd=$db->prepare("UPDATE UTILISATEUR set MOT_DE_PASSE=? WHERE ID_USER = ?");
                        $nvmd->execute([$mdp,$id_user]);
    ...
    <div class="container">
        ...
        <main class="main-content">
            <div class="containerp">
                <h2 class="welcome-header">Welcome, <?=$nom?> <?=$prenom?></h2>
                ...
                <div class="profile-section">
                    ...
                    <div class="profile-infop">
                        <h3 class="profile-namep"><?=$nom?> <?=$prenom?></h3>
                        <p class="profile-emailp"><?=$email?></p>
                    </div>
                ...
                <form id="editForm" class="form-grid" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="select-wrapper">
                            <select class="select-field" disabled name="roles" value="<?=$modpro[7]?>">
                                <option value="<?=$modpro[7]?>" disabled selected><?=$modpro[7]?></option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="password" class="form-control" placeholder="Your new Password" name="mdp" />
                    </div>
