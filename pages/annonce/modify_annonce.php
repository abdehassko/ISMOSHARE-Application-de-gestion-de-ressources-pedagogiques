...
if (!isset($_SESSION['id_user']) || $_SESSION['etat']!= "valide"){
    ...
    extract($_SESSION);
    if (isset($_GET["id"])) {
        $reqm = $db->prepare("SELECT * from annonce where ID_ANNONCE=?");
        $reqm->execute([$_GET["id"]]);
        $modannoce=$reqm->fetch(PDO::FETCH_NUM);
    }
    ...
    if(empty($msg)){
        ...
        try {
            if ($role=='admin' || $role === 'formateur') {
                ...
                $date = date("Y-m-d H:i");
                $reqi=$db->prepare("UPDATE annonce set ID_USER=?,NOM_ANNONCE=?,TYPE_ANNONCE=?,CONTENU_ANNONCE=?,DATE_ANNONCE=? where ID_ANNONCE=?");
                $r=$reqi->execute([$id_user,$forumtitle,$forumcat,$forum,$date,$_GET['id']]);
                if ($r) {header("Location:/ISMOSHARE/pages/annonce/annonce.php?msgsucces=modifee avec succes");exit;}
                else {header("Location:/ISMOSHARE/pages/annonce/annonce.php?msgechec=Erreur dans la modification");exit;}
            }
            else {
                date_default_timezone_set('Africa/Casablanca');
                $date = date("Y-m-d H:i");
                $reqi=$db->prepare("UPDATE annonce set ID_USER=?,NOM_ANNONCE=?,TYPE_ANNONCE=?,CONTENU_ANNONCE=?,DATE_ANNONCE=? where ID_ANNONCE=?");
                $r=$reqi->execute([$id_user,$forumtitle,$forumcat,$forum,$date,$_GET['id']]);
                $update=$db->prepare("UPDATE annonce SET ETAT_VALIDATION_A = 'en attente' where ID_ANNONCE=?");
                ...
                if ($r){header("Location:/ISMOSHARE/pages/annonce/annonce.php?msgsucces=Demande de modification envoyer vers l admin");exit;}
                else {header("Location:/ISMOSHARE/pages/annonce/annonce.php?msgechec=Erreur dans la modification");exit;}
            }
        }
                ...
                <form id="newForumModal" class="modal-overlay" style="display: flex;align-self:center;" method="post">
                    <div class="modal-content">
                        ...
                        <div class="form-group">
                            ...
                            <select class="select-field" name="forumcat" value="<?=$modannoce[3]?>">
                                <option value="" disabled>Choose category</option>
                                <?php
                                $themes=["opportunités","actualités","concours","autres"];
                                foreach($themes as $theme){
                                    $s="";
                                    if($modannoce[3]==$theme) $s="SELECTED";
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="input-field" placeholder="Type editing attention title"
                                name="forumtitle" value="<?=$modannoce[2]?>" />
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="textarea-field" placeholder="Type your question"
                                name="forum"><?=$modannoce[5]?></textarea>
                        </div>
